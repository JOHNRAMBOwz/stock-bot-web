...
with st.expander("🚨 【付费用户必读】每日实战操作 SOP 标准流程 (点击展开/收起)", expanded=True):
    st.markdown("""
    **⚠️ 警告：千万不要看到代码就闭眼乱买！请严格按照以下 4 步铁律执行：**
    * **⏰ 9:25:05【刷新数据】**：集合竞价结束，立刻点击下方两个按钮，拉取最新数据。
    * **🌪️ 9:26:00【看模块一 定大局】**：看跌停家数和连板高度。如果跌停超过20家，或最高连板只有2板，**今日绝对空仓，一股不买！** ；同时观察连板股集中在哪个【所属行业】，这就是今天的主线。
    * **🎯 9:27:00【看模块二 挑金子】**：模块二是机器初筛的强势股。请无情淘汰掉冷门行业的杂毛，**只买与【模块一】主线行业共振的股票**。多只备选时，优先买昨日连板数最高的！
    * **⚔️ 9:30:00【知行合一 杀伐果断】**：参考表格中的 `建议入场价` 开盘买入。买入成交后，立刻在券商软件按 `硬性止损位` 挂好防守条件单。**跌破该价位无条件认亏核按钮；次日不涨停无条件获利了结！**
    """)
